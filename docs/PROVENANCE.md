...
| 14 | BUILDINF.TXT — Ken Silverman's published Build engine & tools documentation (historically distributed with BUILD; consulted via the `doc/buildinf.txt` copy of that file in the JFBuild repository mirror) | Ken Silverman | documentation, not code (used strictly as documentation per the M6.2A brief) | MAP/ART field prose used for the M6.2A UV provenance gate: wall/sprite "repeat's - used to change the size of pixels (stretch textures)"; "panning's - used to align textures or to do texture panning"; and "Z coordinates are all shifted up 4" — independent corroboration of the ratified 16:1 vertical metric (D0016 amendment). The file's embedded engine-API descriptions and example game snippets were NOT used; only the format/field prose was. No UV scaling equation exists in this document | no | mitchellcurrie | 2026-08-26 |
| 15 | BUILD.TXT and BUILD2.TXT — Ken Silverman's published Build editor documentation and notes (historically distributed with BUILD; consulted via the `doc/` copies in the JFBuild repository mirror) | Ken Silverman | documentation, not code (used strictly as documentation per the M6.2A brief) | Editor-level texture facts for the M6.2A UV provenance gate: keypad 2/4/6/8 adjust "Repeat values (smooshiness of the sizes of pixels)"; the `/` reset key "set both repeats of the sprite to the default size of 64" and also resets wall repeats — **default repeat = 64 is published**; paste/ENTER adjust xrepeat "so the pixels of the bitmaps have a square-aspect ratio" (qualitative only); the E key on floors/ceilings: unexpanded smooshes any tile "into the normal 64*64 area", expanded gives "the pixel size ... the same as the normal 64*64 ceiling/floor"; BUILD2 8/24/95: "'E' mode always smooshes the texture size by 2 for any size texture"; wall `O` key: "Normally, walls are oriented from the top". None of these numericize the world-unit size of a texel — the absolute scale facts the M6.2A stop needs are absent | no | mitchellcurrie | 2026-08-26 |
| 16 | Duke Nukem 3D Informational Suite v7.3 (Ryan Lennox, https://infosuite.duke4.net/) — published community editor/mapping documentation | Ryan Lennox (community; credits the classic FAQ writers) | documentation, not code | M6.2A UV provenance gate: floor/ceiling textures are anchored to absolute world coordinates — "ceiling/floor textures remain stationary while sectors are moved (except the ones that have relative alignment set with the R key)" — pinning the default floor/ceiling texture origin to world space, not the sector; power-of-two tile-dimension guidance corroborating periodic tiling. No world-to-texel scale equation exists in this guide | no | mitchellcurrie | 2026-08-26 |
