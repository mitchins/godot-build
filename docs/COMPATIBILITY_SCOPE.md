...
| 0f | Structural triangulation may reject some valid simple polygons: earcut's output is checked against FauxBuild's exact coverage oracle, and if the emitted triangles do not tile the polygon exactly the sector is a fatal structured error rather than wrong geometry. Observed on a tightly wound spiral (independently verified valid: zero proper self-crossings, non-zero area); not observed on any of the 2450 sectors across six legally owned maps | Accepted, bounded (D0017) | FauxBuild fails closed rather than emit incorrect geometry. The triangulator is explicitly not claimed to be universal. A rejected sector names itself in the error; if real content ever hits this, the fix is a better triangulation stage, never relaxing the oracle |
